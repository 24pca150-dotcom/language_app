...
@Component({
  ...
  templateUrl: './student-lesson-player.html',
  styleUrls: ['./student-lesson-player.css']
})
export class StudentLessonPlayer implements OnInit, OnDestroy {
  ...
  startAssessment(step: any) {
    let id = null;
    if (Array.isArray(step.data) && step.data.length > 0) {
      id = step.data[0].id;
    } else if (step.data && step.data.id) {
      id = step.data.id;
    }
    if (id) {
          ...
          if (block.type === 'paragraph') return `<div class="opacity-75 mb-4">${block.data.text}</div>`;
            ...
            rows.forEach((row: string[], index: number) => {
              if (index === 0 && withHeadings) {
                html += `<thead style="background: #e0e7ff;"><tr>` + row.map(cell => `<th class="p-2 text-dark fs-5 fw-bold border-bottom-0">${cell}</th>`).join('') + `</tr></thead><tbody>`;
              } else {
      ...
      if (state === 'correct') {
        setTimeout(() => {
          gsap.fromTo('.stat-badge',
            { scale: 1.3, boxShadow: '0 0 20px #4f46e5' },
  ...
  speakText(text: string | null) {
    ...
    sentences.forEach((sentence, idx) => {
      ...
      utterance.rate = isTamil ? 0.85 : 0.90;
      utterance.pitch = 1.0; // Adult/academic pitch
      ...
      utterance.onend = () => {
        ...
        if (this.activeUtterances.length === 0) {
          this.isSpeaking.set(false);
        }
      };
      utterance.onerror = () => {
